...
public class MobileSmelting implements BaseListener {
	private PlayerHandler<MCCustomPlayer> player_handler;
	private UtilityManager utility_manager;

	public MobileSmelting() {
		player_handler = MC_Custom_Classes.getPlayerHandler();
		this.utility_manager = MC_Custom_Classes.getUtilityManager();
	}

	@EventHandler
	public void mobileSmelt(final PlayerInteractEvent event) {
		Player player = event.getPlayer();
		try {
			if (/*Replace this with whatever check for mobile smelting*/true) {
				if (player_handler.getPlayer(player).hasAbilityInRegion(AbilityType.MOBILE_SMELTING)) {
